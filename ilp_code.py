...
ca = ones.copy()
ca = ca * (number_of_company/number_of_reps*1.1)

cal = ones.copy()
cal = cal * (-number_of_company/number_of_reps*0.9)
...
cs = ones.copy()
cs = cs*(np.sum(s)/number_of_reps*1.1)

csl = ones.copy()
csl = csl * ( - np.sum(s)/number_of_reps*0.9)
...
allocation.to_csv('output.csv', sep=',', header=False, index=False)
#f = open('output.csv','w')
#f.write(str(result))
#f.close()
